...
Функция УдалитьНастройкиДобавить(Запрос)
	...
	Попытка
		...
		ТекущийПользователь = Пользователи.ТекущийПользователь();
		//КЕА_РегистрыСведений.АвторизацияВСервисе1СФреш.УдалитьЗапись(ТекущийПользователь);
...
Функция СостояниеУстановкиУдаленияНастроекПолучить(Запрос)
	...
	//КЕА_Ответ.УстановитьТелоИзСтроки(ОплатаСервиса.СтрокаJSON(ДанныеОтвета));
...
Функция ШаблонСостоянияЗадания()
	...
	//КЕА_ШаблонДанныхЗадания = Новый Структура;
	//КЕА_ШаблонДанныхЗадания.Вставить("id", "");
	//КЕА_ШаблонДанныхЗадания.Вставить("status", "");
